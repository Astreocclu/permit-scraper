...
Run:
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/etrakit_fast.py flower_mound 50
```
...
Run:
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 -c "import json; d=json.load(open('flower_mound_raw.json')); print(f'Count: {d[\"actual_count\"]}'); print(f'Sample: {d[\"permits\"][0] if d[\"permits\"] else \"none\"}')"
```
...
Run:
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/etrakit_fast.py flower_mound 1000
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && git add flower_mound_raw.json && git commit -m "data: add Flower Mound permit data (eTRAKiT)"
```
...
Run:
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/cityview.py carrollton 20
```
...
Run:
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/cityview.py carrollton 500
```

**Step 5: Commit**

```bash
cd /home/astre/command-center/src/greenlit/collections && git add scrapers/cityview.py carrollton_raw.json && git commit -m "feat: add CityView scraper for Carrollton"
```

---

## Task 3: Update docs/archive/SCRAPER_STATUS.md
...
```bash
cd /home/astre/command-center/src/greenlit/collections && git add docs/archive/SCRAPER_STATUS.md && git commit -m "docs: update scraper status with new coverage"
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && mkdir -p _archive/2025-12-11 && mv scrapers/mygov.py _archive/2025-12-11/
```

**Step 2: Commit**

```bash
cd /home/astre/command-center/src/greenlit/collections && git add -A && git commit -m "chore: archive mygov.py (requires login, not scrapeable)"
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && for city in dallas fort_worth frisco flower_mound; do
  echo "=== $city ==="
...
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 -c "import json; d=json.load(open('carrollton_raw.json')); print(f'Carrollton: {d.get(\"actual_count\", 0)} permits')"
```
