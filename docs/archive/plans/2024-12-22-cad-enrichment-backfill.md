...
```bash
cd /home/astre/command-center/src/greenlit/collections && pytest tests/test_backfill_cad.py -v
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && pytest tests/test_backfill_cad.py::TestGetCountyForCity -v
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && pytest tests/test_backfill_cad.py::TestFetchUnenrichedPermits -v
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 -c "from scripts.backfill_cad_enrichment import query_cad_with_retry, COUNTY_CONFIGS; print('OK')"
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && pytest tests/test_backfill_cad.py::TestUpsertLeadsProperty -v
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 -c "from scripts.backfill_cad_enrichment import process_permit; print('OK')"
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 scripts/backfill_cad_enrichment.py --help
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 scripts/backfill_cad_enrichment.py --dry-run --limit 5 --city frisco
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && PGPASSWORD=localdev123 psql -h localhost -U contractors_user -d contractors_dev -c "
...
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 scripts/backfill_cad_enrichment.py --limit 10 --city frisco
```
...
```bash
cd /home/astre/command-center/src/greenlit/collections && nohup python3 scripts/backfill_cad_enrichment.py > logs/backfill_$(date +%Y%m%d_%H%M%S).log 2>&1 &
...
```bash
# Check progress
tail -20 /home/astre/command-center/src/greenlit/collections/logs/backfill_*.log
...
```bash
cd /home/astre/command-center/src/greenlit/collections && python3 scripts/score_leads.py --rescore
```
...
Update `docs/archive/LEADS_INVENTORY.md` with new counts if significant improvement.
