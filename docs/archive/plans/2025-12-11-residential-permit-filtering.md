...
Run: `cd /home/astre/command-center/src/greenlit/collections && python3 -m pytest tests/test_filters.py -v`
...
```bash
cd /home/astre/command-center/src/greenlit/collections && git add scrapers/filters.py tests/test_filters.py && git commit -m "feat: add residential permit filter with TDD tests"
```
...
Run: `cd /home/astre/command-center/src/greenlit/collections && grep -n "save\|write\|export\|json.dump" scrapers/citizen_self_service.py`
...
Run: `cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/citizen_self_service.py --city southlake --days 30 --dry-run`
Expected: Should show "Filtered to X residential permits" in output

**Step 5: Commit**

```bash
cd /home/astre/command-center/src/greenlit/collections && git add scrapers/citizen_self_service.py && git commit -m "feat: integrate residential filter into Southlake CSS scraper"
```
...
Run: `cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/westlake_spike.py`
Expected: Either:
...
```bash
cd /home/astre/command-center/src/greenlit/collections && git add scrapers/westlake_spike.py && git commit -m "spike: verify Westlake MyGov autocomplete endpoint"
```
...
Run: `cd /home/astre/command-center/src/greenlit/collections && python3 -m pytest tests/test_westlake_harvester.py -v`
...
```bash
cd /home/astre/command-center/src/greenlit/collections && git add scrapers/westlake_harvester.py tests/test_westlake_harvester.py && git commit -m "feat: add Westlake address harvester with autocomplete"
```
...
Run: `cd /home/astre/command-center/src/greenlit/collections && head -100 scrapers/mygov_westlake.py`
...
Run: `cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/mygov_westlake.py --dry-run`
Expected: Should show "Loaded X harvested addresses" or fallback message

**Step 5: Commit**

```bash
cd /home/astre/command-center/src/greenlit/collections && git add scrapers/mygov_westlake.py && git commit -m "feat: use harvested addresses in Westlake scraper"
```
...
Run: `cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/citizen_self_service.py --city southlake --days 30`
Expected:
- Scrapes permits
...
Run: `cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/westlake_harvester.py`
Expected:
...
Run: `cd /home/astre/command-center/src/greenlit/collections && python3 scrapers/mygov_westlake.py`
Expected:
...
Run: `cd /home/astre/command-center/src/greenlit/collections && python3 -c "import json; d=json.load(open('data/raw/southlake_raw.json')); types=set(p.get('type','') for p in d); print('\\n'.join(sorted(types)))"`
Expected: Only residential permit types (no Commercial, Business, Sign, etc.)

**Step 5: Final commit**

```bash
cd /home/astre/command-center/src/greenlit/collections && git add -A && git commit -m "feat: complete residential filtering for Southlake and Westlake"
```
...
Run: `cd /home/astre/command-center/src/greenlit/collections && echo "Download Westlake streets from OSM or use manual list"`
